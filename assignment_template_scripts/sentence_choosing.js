        ...
        function ($, tools, sentencesList, audioLoader) { //, fixedProblemHolder
            ...
            tools.getProblemData(function (d) {
                ...
                audio = audioLoader({
                    ...
                    playButton: "playIcon",
                    onAllLoaded: prepareProblems, // moving on AFTER all the sounds are loaded
            ...
            var nextProblem = (function () {
                ...
                return function () {
                    ...
                    currentProblem.choices.forEach(function (english, index) {
                        $("#myRadioLabel" + (index)).text(english);
                    });
            ...
            function checkAnswer() {
                ...
                console.log(currentProblem.indexOfCorrectAnswer - 1);
                var $correctAnswer = $(".myRadio[id='choice" + currentProblem.indexOfCorrectAnswer - 1 + "']");
                ...
                if ($checkedRadio.is($correctAnswer)) {
                    ...
                    $(".myRadioLabel").not("#myForm label[for='choice" + (currentProblem.indexOfCorrectAnswer - 1) + "']").closest("tr").addClass("faded");
            ...
            function displayChoices() {
                ...
                $("#fadeThing").empty().append("<table id='myTable' />");


                // appending the rows & cells for the table
                for (var i = 0; i < currentProblem.numberOfSentakushi; i++) {


                    // building the radio input
                    var $radioHolderTD = $("<td class='radioHolder' />");
                    ...
                    // appending the radio stuff to the radio holder
                    $radioHolderTD.append($myRadio);


                    // preparing the label and adding the value (the Japanese choice)
                    var $labelTD = $("<td />");
                    var $label = $("<label class='myRadioLabel' />").prop({
                        id: "myRadioLabel" + i,
                        for : "choice" + i
                    }).text(currentProblem.choices[i]);
